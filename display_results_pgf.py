...
def learning_curve_reward_average(agent_names, legend, legend_title):
    ...
    # ax.set_yticks(ticks=[-10, -5, 0, 5], labels=[-10, -5, 0, 5])
   

    for i in range(np.size(end_ep, axis=0)):
        end_episode = end_episodes[i] 
        ax.plot(np.arange(0,end_episode,100), np.array(avg_score[i])[np.arange(0,end_episode,100)])
        ax.fill_between(x=np.arange(0,end_episode,100), y1=np.array(score_upper_fill[i][0])[np.arange(0,end_episode,100)], y2=np.array(score_lower_fill[i][0])[np.arange(0,end_episode,100)], alpha=0.15, label='_nolegend_')
    ...
    fig.tight_layout()
    fig.subplots_adjust(bottom=bottom_space, left=0.25, right=0.75) 
    plt.figlegend(legend, title=legend_title, loc = 'lower center', ncol=5)
    plt.show()
    # plt.savefig('results/'+filename+'.pgf', format='pgf')
...
def display_path_velocity_colormap(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, start_condition):
    ...
    for agent_name, n, i in zip(agent_names, ns, range(len(agent_names))):
        ...
        # Compensate for changes to reward structure
        env_dict['reward_signal']['max_progress'] = 0
        # env_dict['steer_control_dict']['track_width'] = 0.9
...
def display_path_slip_colormap(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, start_condition):
    ...
    # cmap = plt.get_cmap('rainbow')
    cmap_settings = ('Spectral', None)
    cmap = symmetrical_colormap(cmap_settings= cmap_settings, new_name =None )
    ...
    for i in range(len(agent_names)):
        ...
        for i in range(1, len(state_history[0])):
            ...
            ax.plot([x0, x1], [y0, y1], '-', color=cmap(norm(state_history[0][i][6])), linewidth=1, alpha=0.8)
...
# start_condition = {'x':4.1, 'y':9, 'v':3, 'theta':0, 'delta':0, 'goal':0}
# start_condition = []
# display_path_slip_colormap(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, start_condition)


def display_path_mismatch_compare_colormap(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, start_condition):
    ...
    for i in range(len(agent_names)):
        ...
        for i in range(1, len(state_history[0])):
            x0, y0 = state_history[0][i-1][0], state_history[0][i-1][1]
            x1, y1 = state_history[0][i][0], state_history[0][i][1]
            ax.plot([x0, x1], [y0, y1], '-', color=cmap(norm(np.abs(state_history[0][i][6]))), linewidth=1, alpha=0.8)
