...
def learning_curve_lap_time_average(agent_names, legend, legend_title, ns):
    ...
    for i in range(len(agent_names)):
        ...
        ax[0].fill_between(x=np.arange(end_episode), y1=upper_fill_coll[i][0][0:end_episode], y2=lower_fill_coll[i][0][0:end_episode], alpha=0.15, label='_nolegend_')
    ...
    for i in range(np.size(end_ep, axis=0)):
        ...
        #plt.plot(np.cumsum(steps[0]), steps[0], 'x')
        #plt.plot(steps[0][np.where(np.logical_not(collisions[0]))], 'x')
        ax[1].plot(np.arange(end_episode), np.array(steps_y_avg_smoothed[i][0:end_episode])*0.01)
        ax[1].fill_between(x=np.arange(end_episode), y1=upper_fill[i][0][0:end_episode]*0.01, y2=lower_fill[i][0][0:end_episode]*0.01, alpha=0.15, label='_nolegend_')
    ...
    plt.figlegend(legend_coll, loc = 'lower center', ncol=4, labelspacing=0.)
...
agent_names = ['porto_ete_r_1', 'porto_ete_r_2']
legend = ['1', '0.7']
legend_title = ''
ns=[0, 0]

learning_curve_lap_time_average(agent_names, legend, legend_title,ns)