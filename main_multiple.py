...
def lap_time_test_unknown_mass(agent_name, n_episodes, detect_issues, initial_conditions, mass, distances):
   ...
   results_dir = 'lap_results_mismatch/' + agent_name
   results_file = results_dir + '/' + 'unknown_mass'
   parent_dir = os.path.dirname(os.path.abspath(__file__))
   agent_dir = parent_dir + '/agents/' + agent_name
   agent_params_file = agent_dir + '/' + agent_name + '_params'
   replay_episode_name = 'replay_episodes/' + agent_name
   ...
   param_dict = {'parameter': 'payload mass'
               , 'mass': mass
               , 'distances': distances
               , 'times_results': np.zeros((runs, len(distances), n_episodes))
               , 'collision_results': np.zeros((runs, len(distances), n_episodes))
               }

   for v_i, distance in enumerate(distances):
      
      car_params = init_car_params.copy()
      
      m_new = car_params['m'] + mass
      lf_new = (car_params['m']*car_params['lf']+mass*distance) / (m_new)
      I_new = car_params['I'] + car_params['m']*abs(lf_new-car_params['lf'])**2 + mass*abs(lf_new-distance)**2
      ...
      for n in range(runs):
         ...
         a.load_weights(agent_name, n)
         print("Testing agent " + agent_name + ", n = " + str(n) + ", payload distance = " + str(round(distance,2)))
