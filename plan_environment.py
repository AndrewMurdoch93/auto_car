...
class environment():
    ...
    def take_action(self, act):
        ...
        waypoint, wpt_angle, R, self.v_ref = self.convert_action_to_coord(strategy=self.waypoint_strategy, action=act)
        ...
        while (lastIndex > target_index) and i<40:
            ...
            self.update_pose(delta_ref, self.v_ref)
    ...
    def convert_action_to_coord(self, strategy, action):
        ...
        if self.action_space=='discrete':
            ...
            return waypoint, wpt_angle, R, v_ref
    ...
    def update_pose_plan(self, steer, vel, state):
        #assume ete agent
        accl = vel
        
        _, sv = vehicle_model.pid(vel, steer, state[3], state[2], self.params['sv_max'], self.params['a_max'], self.params['v_max'], self.params['v_min'])
...
def test_environment():
    ...
    env_dict['path_dict']['path_strategy']='ete_control'
    ...
    action_history = np.ones((1000,2))*0.7
    ...
    while done==False:
        ...
        while fail==False and plan_step<=10:
