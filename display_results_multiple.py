...
def display_lap_mismatch_results_multiple_mu(agent_names, parameters, legend_title, legend, plot_titles, nom_value, graph, text):
    ...
    fig, axs = plt.subplots(nrows=1, ncols=1, figsize=(5.5,2.5))
    ...
    axs.vlines(x=0.53, ymin=-50, ymax=150, color='black', linestyle='--')
    ...
    fig.tight_layout()
    fig.subplots_adjust(bottom=0.32, left=0.2, right=0.8) 
...
def display_lap_mismatch_results_multiple_mu_2(agent_names, legend):
    ...
    for agent in agent_names:
        ...
        for i in range(n_param):
            ...
            if agent=='f1_esp_pete_r_p_5':
                avg_col[i] = np.round(np.sum(np.logical_not(results_dict['collision_results'][np.array([0,2]),i,:]))/(n_episodes*2), 2)
                failures = np.count_nonzero(results_dict['collision_results'][np.array([0,2]),i,:].flatten())
                successes = n_episodes*n_runs - failures
                dev_col[i] = np.sqrt(n_episodes*(successes/n_episodes)*((failures)/n_episodes))/(n_episodes*2)
            
            elif agent=='f1_mco_pete_mu_4':
                avg_col[i] = np.round(np.sum(np.logical_not(results_dict['collision_results'][np.array([0,2]),i,:]))/(n_episodes*2), 2)
                failures = np.count_nonzero(results_dict['collision_results'][np.array([0,2]),i,:].flatten())
                successes = n_episodes*n_runs - failures
                dev_col[i] = np.sqrt(n_episodes*(successes/n_episodes)*((failures)/n_episodes))/(n_episodes*2)
        ...
        elif agent=='porto_pete_sv_p_r_0' or agent=='f1_esp_pete'or agent=='f1_mco_pete_mu_4':
    ...
    for i in [0,1]:
        ...
        axs[i].text(x=0.45, y=10, s='   Wet\nasphalt', fontsize = 'small', verticalalignment='center', bbox=dict(facecolor='white', edgecolor='black',pad=0.1,boxstyle='round'))

    axs[0].set_title('End-to-end')
    axs[1].set_title('Partial end-to-end')
...
agent_names = ['porto_ete_v5_r_collision_5', 'f1_esp_ete', 'f1_mco_ete', 'porto_pete_sv_p_r_0', 'f1_esp_pete_r_p_5', 'f1_mco_pete_mu_4']
# parameters = ['mu', 'C_S']
# nom_value = [1.0489, 1]
...
def display_lap_unknown_mass(agent_names, legend):
    ...
    for agent in agent_names:
        ...
        std_dev_filter = functions.savitzky_golay(dev, 9, 2)

        axs.plot(results_dict['distances'], avg_filter*100, alpha=0.8)
        # axs.plot(results_dict['distances'], std_dev_filter*100, alpha=0.2, label='_nolegend_')
...
def eval_lap(agent_names, ns, mismatch_parameters, frac_vary, noise_dicts, start_condition):
...
def display_path_multiple(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, noise_dicts, start_condition):
    ...
    for i in range(len(agent_names)):
        ...
        ax.plot(np.array(state_history[i])[:,0], np.array(state_history[i])[:,1], linewidth=1, alpha=alpha)   
...
# agent_names = ['porto_ete_v5_r_collision_5', 'porto_pete_s_polynomial', 'porto_pete_v_k_1_attempt_2', 'porto_pete_sv_p_r_0']    
# agent_names = ['porto_ete_v5_r_collision_5', 'f1_esp_ete', 'f1_mco_ete', 'porto_pete_sv_p_r_0', 'f1_esp_pete_r_p_5', 'f1_mco_pete_mu_4']
# agent_names = [ 'f1_esp_ete',  'f1_esp_ete', 'f1_esp_pete_r_p_5', 'f1_esp_pete_r_p_5']
...
def display_path_mismatch_multiple_by_agent(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, noise_dicts, start_condition):
    ...
    if True:
        ...
        fig, ax =   plt.subplots(nrows=1, ncols=2, figsize=(5.5,2.8))
        ...
        ax[0].set_title('End-to-end')
        ax[1].set_title('Partial end-to-end')
        fig.tight_layout()
...
def display_path_mismatch_multiple_by_agent_4(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, noise_dicts, start_condition):
...
agent_names = ['porto_ete_v5_r_collision_5', 'porto_pete_s_polynomial', 'porto_pete_v_k_1_attempt_2', 'porto_pete_sv_p_r_0']    
legend = ['No added mass', 'Mass placed above front axle']
legend_title = ''
ns=[2,1,0,0]
...
def display_path_multiple_2(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, noise_dicts, start_condition):
    ...
    fig, ax = plt.subplots(2, 1, figsize=(5,4), gridspec_kw={'height_ratios': [2, 1]})
    
    ax[0].axis('off')
    
    # ax[n_ax].tick_params(axis='both', colors='lightgrey')

    for n_ax in [0,1]:
        ax[n_ax].spines['bottom'].set_color('lightgrey')
        ax[n_ax].spines['top'].set_color('lightgrey') 
        ax[n_ax].spines['right'].set_color('lightgrey')
        ax[n_ax].spines['left'].set_color('lightgrey')
    ...
    for i in range(len(agent_names)):
        ...
        ax[0].plot(np.array(state_history[i])[:,0], np.array(state_history[i])[:,1], linewidth=1, alpha=alpha)   
        # ax.plot(np.array(pose_history[i])[:,0][np.arange(0,len(local_path_history[i]),40)], np.array(pose_history[i])[:,1][np.arange(0,len(local_path_history[i]),40)], 'x')
    ...
    for i in range(len(agent_names)):
        ax[1].plot(np.array(progress_history[i])*100, np.array(state_history[i])[:,6], linewidth=1.5, alpha=alpha)
...
def display_path_twice_multiple(agent_names, ns, legend_title, legend, mismatch_parameters, frac_vary, noise_dicts, start_condition):
    ...
    for i in range(len(agent_names)):
        ...
        #         ax.plot(j[0][0], j[1][0], alpha=0.5, color='red', marker='s')
        if i==0:
            ax[0].plot(np.array(state_history[i])[:,0], np.array(state_history[i])[:,1], linewidth=1, alpha=alpha, color='grey', linestyle='--')   
        
        if i==1 :
            ax[1].plot(np.array(state_history[i])[:,0], np.array(state_history[i])[:,1], linewidth=1, alpha=alpha, color='blue')   

        if i==2:
            ax[0].plot(np.array(state_history[i])[:,0], np.array(state_history[i])[:,1], linewidth=1, alpha=alpha, color='orange')   
        
        if i==3: 
            ax[1].plot(np.array(state_history[i])[:,0], np.array(state_history[i])[:,1], linewidth=1, alpha=alpha, color='green')   
